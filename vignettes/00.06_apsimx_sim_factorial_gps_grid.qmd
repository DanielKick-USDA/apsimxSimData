...
soils_df[((soils_df$Longitude == 0) &
          (soils_df$Latitude == 0)), c('Longitude', 'Latitude')] <- NA
...
#   )
# 
# unlink(paste0(output_dir, 'temp.apsimx'))
...
modify_and_run_apsimx_temp <- function(
  ...
  # basic version
  soils_i <- closest_soils_list_entry(ith_lon = ith_lon, ith_lat = ith_lon)
  soils_list_i <- as.character(soils_df[soils_i, 'soils_list_i'])
  ...
  # and also copy the file so that I can audit it and figure out why it did not work.
    ...
    file.copy(paste0(cache_path, 'temp.apsimx'),
              paste0(cache_path, 'apsimx_audits/',  paste(c(ith_lon, ith_lat, ith_year_start, ith_year_end, soils_list_i, 'temp.apsimx'), collapse = '__' )))
  }
  ...
  return(cbind(data.frame(ith_lon, ith_lat, ith_year_start, ith_year_end, soils_list_i, factorial_file), res))
}
```
...
modify_and_run_apsimx_factorial <- function(
  ...
  # and also copy the file so that I can audit it and figure out why it did not work.
    ensure_dir_path_exists(paste0(cache_path, 'apsimx_audits/'))
    file.copy(paste0(cache_path, factorial_temp_file),
              paste0(cache_path, 'apsimx_audits/',  paste(c(ith_lon, ith_lat, ith_year_start, ith_year_end, factorial_temp_file), collapse = '__' )))
  }
...
Check if all g2f sites worked
```{r}

gps %>% head()

temp <- list.files("../data/00.06_apsimx_sim_factorial_gps_grid/factorial_exps/")

gps_counts <- do.call(rbind, map(1:nrow(gps), function(i){
  mask <- str_detect(temp, paste0(gps[i, 'lon'], '__', gps[i, 'lat']))
  nvals <- nrow(read.csv(paste0("../data/00.06_apsimx_sim_factorial_gps_grid/factorial_exps/", temp[mask])))
  return(data.frame(lon = gps[i, 'lon'], lat = gps[i, 'lat'], nvals = nvals))
}))


gps_counts %>% 
  mutate(Pass = nvals > 1) %>% 
  ggplot(aes(lon, lat, color = Pass))+
  geom_point()


filter(gps_counts, nvals == 1)
...
# I'm pausing this avenue. The issue is that the soil data needs to be edited before it can run. This appears to be a _very_ common problem. Here are some options:
# 1. Validate soil/weather before use. 
# 1.a. Test each separately and then mix and match soil/weather to get hypothetical yields in non-existant combiantions
# 2. Edit soil on the fly
# 2.a. This is the approach I have been trying. The SW parameter in the soil table is not editable directly. If I try editing the air-dry then apsimx fails with an error I havent' figured out. 



# # fails
# modify_and_run_apsimx_factorial(
#     ith_lon = -96.42790,
#     ith_lat = 30.53826,
#     cache_path = "../data/00.06_apsimx_sim_factorial_gps_grid/",
#     factorial_file = 'SimulateFactorial_Tiny.apsimx',
...
# # inspect file and see if I can find the negative values. 
# 
# '../data/00.06_apsimx_sim_factorial_gps_grid/factorial_exps/'
# 
# 
# temp <- jsonlite::read_json('../data/00.06_apsimx_sim_factorial_gps_grid/apsimx_audits/-96.4279__30.53826__1984__2022__SimulateFactorial_Tiny_temp_soil.apsimx')
# 
# 
# 
# temp %>% str(max.level = 2)
# 
# temp$Children[[2]]$Children %>% str(max.level = 2)
# 
# 
# temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]] %>% str(max.level = 2)
# 
# soil_module <- temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]] 
# soil_module %>% str(max.level = 2)
# 
# 
# ## ----
# # might be air-dry value?
# 
# # this looks like LL15
# soil_module$Children[[1]]$Children[[3]]$LL
# 
# 
# soil_module$Children[[1]]$Children[[3]]
# 
# # list of 10 -- one for each layer?
# soil_module$Children[[2]] %>% str(max.level = 2)
# 
# 
# walk(c(
#   1, 
...
#   # 10
# ), function(i){
#   print(paste0(i, paste(rep('-', times = 78), collapse = '')))
#   print(
#   soil_module$Children[[i]] %>% str(max.level = 2)
# )})
# 
# # 1 and maybe 6?
# 
...
# 
# 
# soil_module$Children[[1]]$AirDry 
# soil_module$Children[[6]]$InitialValues 
# 
# 
# # check if soil water in layer is below air-dry value
# soil_layers <- min(length(
#   temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[1]]$AirDry 
# ),
# length(temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[6]]$InitialValues))
# 
# # ith_layer = 1
# for(ith_layer in seq(1, soil_layers)){
#   print(ith_layer)
#   # if soild water is below air-dry, overwrite with air-dry
#   if( temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[6]]$InitialValues[[ith_layer]] <
#       temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[1]]$AirDry[[ith_layer]] ){
#     
#     temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[6]]$InitialValues[[ith_layer]] <- temp$Children[[2]]$Children[[1]]$Children[[2]]$Children[[5]]$Children[[2]]$Children[[1]]$AirDry[[ith_layer]]
#     
#   }
# }
# 
# 
# 
# jsonlite::write_json(temp, 
#                      "../data/00.06_apsimx_sim_factorial_gps_grid/apsimx_audits/test_changes.apsimx", 
#                      pretty = TRUE, 
#                      digits = NA, auto_unbox = TRUE, null = "null", 
#                      na = "null")
```
